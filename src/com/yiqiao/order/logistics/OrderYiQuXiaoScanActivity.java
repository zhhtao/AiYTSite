...
public class OrderYiQuXiaoScanActivity extends FinalActivity{
	@ViewInject(id=R.id.order_yi_qu_xiao_scan_progress_bar_layout)private RelativeLayout progressBarLayout;
	@ViewInject(id=R.id.order_yi_qu_xiao_scan_show_info_layout)private LinearLayout showInfoLayout;
	@ViewInject(id=R.id.order_yi_qu_xiao_scan_title_back)private ImageView backImageView; 
	@ViewInject(id=R.id.order_yi_qu_xiao_scan_listview)private ListView listView;
	
	//该活动采用了已完成活动的 listview—item 和 Adapter 
	...
	private OrderYiWanChengScanAdapter adapter;//数据填充适配器
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.order_yi_qu_xiao_scan_layout);
		mContext = this;
	...
	private void qureyDataFromServer() {
		...
		for (int i=0; i<10; i++) {
			...
			orderTotalInfo.setOrderState(MyConstants.ORDER_STATE_YIQUXIAO);
...
//			orderInvoiceInfo.setReceiveTime("2015-6-20 14:28");
			...
			//订单取消原因
			orderTotalInfo.setOrderCancleReason("卖家无货了");
	...
	private class MyOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			// TODO Auto-generated method stub
			switch (v.getId()) {
			case R.id.order_yi_qu_xiao_scan_title_back:
				back();
				break;
