...
public class OrderQuXiaoShengQingScanActivity extends FinalActivity{
	...
	private OrderQuXiaoShenQingScanAdapter adapter;//数据填充适配器
	...
	protected void  onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == 1)//同意取消
		{
			if (resultCode == RESULT_OK) {//有订单取消
				...
				//取消订单后操作 更改状态  更新Listview
				OrderTotalInfo orderTotalInfo = orderTotalInfosList.get(position);
				orderTotalInfo.setOrderState(MyConstants.ORDER_STATE_YIQUXIAO);//更改状态为已取消订单
				//上传服务器更新
		...
		} else if (requestCode == 2) { //不同意取消
			if (resultCode == RESULT_OK) {
				int position = data.getIntExtra("selected_positon", -1);
				String cancleReason = data.getStringExtra("cancle_reason");
				System.out.println(cancleReason);
				
				//不同意取消订单后操作 更改状态  更新Listview
				OrderTotalInfo orderTotalInfo = orderTotalInfosList.get(position);
				orderTotalInfo.setOrderState(MyConstants.ORDER_STATE_WEIFAHUO);//更改状态为未发货订单
				//上传服务器更新
	...
	private void initView() {
		...
		adapter = new OrderQuXiaoShenQingScanAdapter(mContext, orderTotalInfosList);
	...
	private void qureyDataFromServer() {
		...
		for (int i=0; i<10; i++) {
			...
			orderTotalInfo.setOrderState(MyConstants.ORDER_STATE_QUXIAO_SHENAQINF);
